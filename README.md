...
The design's other examples (`chat-cli`, `agui-server`, `mcp-memory`, `structured`) arrive with the phases they exercise; `local-ollama` is folded into `examples/providers` above, since the example that selects a provider at runtime is already the one that has an Ollama to talk to.
