...
Status: **all eight phases are built and merged.** The codec seam with five providers, tool calling from function signatures, streaming with one code path, structured output with bounded repair, guardrails at four stages, the MCP client with memory-as-a-server, AG-UI over SSE, and the `@prompt` tier with automatic cache breakpoints. Every toolchain prerequisite this document named has landed (§17); the questions are settled or corrected in place, and the corrections are kept rather than edited away.
...
- **The wall-clock deadline is opt-in, and it kills the server.** The design did not name a timeout; building it found there is no cheap one. A blocked `Process.read_line` parks the whole scheduler, only an `isolate` runs beside it, `concurrent` is illegal in a synchronous `fn`, `ToolSource.call` must stay synchronous (it is `dyn`, and O-2), and the `Process` handle is not `Send` so the kill goes out through `kill(1)` by pid — all measured, all in AGENTS.md. So `Stdio.with_deadline(ms)` is an isolate watchdog, disarmed by `h.cancel()` when the reply arrives, terminal when it fires, and off by default; the crash path needs none of it, and the *always-on* bound is a read budget. The watchdog sleeps its deadline in 5 ms slices because that is what makes the cancel prompt: an isolate parked in one long `sleep` observes a cancel only when that sleep ends. HTTP needs none of it either: `client.timeout(ms)` already bounds a request.
...
- ~~**The schema arrives as a value, because a generic body cannot ask what `T` is called.**~~ **Closed by noeta 0.3.** `type_name::<T>()` over a type parameter was E0058, so a call site had to reflect where the type was concrete and pass the result in — `Output.of(type_name::<Extraction>())`, naming the type twice. The toolchain change this entry predicted ("a name is all it asks for") landed: a generic forwards its own type parameter into `type_name::<T>()`, and a generic *method* may do it too. `Output.of::<Extraction>()` names the type once. The string-keyed door survives as `Output.for_name(target, about)` for an identity computed at run time rather than named in source.
