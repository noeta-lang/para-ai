...
- `noeta check <file>.noe` and `noeta test <file>.noe` at the repo root run the package's own suite — the codec fixture tables, the delta fold, the base64 vectors, the captured-SSE decoder tables, and the end-to-end run-loop tests over `Mock` on both transports. 120 tests across the five files as of phase 3.
- **Check from an example, not just from the package root.** A module that is the entry links differently from one reached through an import, and at least one real check error (the `dyn` coercion below) appears *only* from a consuming package. `noeta test` in `examples/ask` is the cheapest way to catch it.
...
- **A module-qualified extern enum variant does not compile.** `use std.http` then `http.Framing.Sse` type-checks and then dies with `internal error: the VM cannot compile this program: type member used as a value` — no source location, nothing to grep. `use std.http.{Framing}` and a bare `Framing.Sse` works. This bites hardest where two enums share a name, which `std.http.Framing` and `para.ai.provider.Framing` do: no module may name both, so `Wire` exposes `streamed()` / `ndjson()` / `as_sse()` / `as_ndjson()` and the run loop reconstructs std's enum at the one place it calls `client.stream`.
- **A whole-module import shadows pattern bindings, and the failure is at run time.** `use std.id` binds `id`, and every `Part.ToolCall(id, name, args)` arm in the file then aborts with `cannot assign to \`id\`, which is immutable` — no check-time error, no location. The field-collision case is E0020; the pattern-binding case is not caught at all. `use std.id.{uuid}` instead.
...
Each was reproduced minimally against `lang` at `7575aece`. They are worked around here rather than papered over. Two are **now fixed in `lang` main** but are not in a released toolchain yet, and CI pins a release — so their workarounds stay until the `toolchain` floor in `noeta.toml` rises past them, at which point delete the workaround and the note together.
...
- **A standalone `impl Trait for Type` is invisible to `dyn` coercion when its module is not the entry** — see the gotcha above for the shape and the fix. Bisected in this package rather than reduced: removing `fn decoder(): dyn StreamDecoder` from the `Provider` trait makes the standalone impls resolve again, and writing the impls inline makes the trait method fine. A three-file reduction (trait in one module, class plus standalone impl in another, a second trait naming `dyn T`, a generic struct bounded by it, an entry importing everything) did **not** reproduce, so something more of this package's shape is involved. Worth reducing properly before filing.
- **A module-qualified extern enum variant is an unlocated VM internal error.** Minimal repro, whole program:

  ```noeta
  use std.http
  fn a(): http.Framing {
      return http.Framing.Sse
  }
  ```

  The checker accepts it; the VM reports `type member used as a value` with no span. The checker should either resolve the qualified path or reject it with a located diagnostic.
